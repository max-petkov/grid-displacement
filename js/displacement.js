class ImageDisplacement {
  constructor(imgElement) {
    ...
    if (this.image.complete && this.image.naturalHeight !== 0) {
      ...
      this.image.addEventListener("error", () =>
        console.error("Failed to load:", this.image.src),
      );
    }
  }
  ...
  initGPGPU() {
    ...
    this.gpgpuRenderer = new THREE.GPUComputationRenderer(
      this.gpgpuSize,
      this.gpgpuSize,
      this.renderer,
    );
    ...
    const gpgpuShader = `
            ...
            void main() {
                ...
                // THE FIX: Calculate distance ONLY on the X-axis!
                // This creates a vertical brush stroke that spans the entire height.
                float dist = abs(uv.x - uMouse.x);
    ...
    this.variable = this.gpgpuRenderer.addVariable(
      "uGrid",
      gpgpuShader,
      this.dataTexture,
    );

    this.variable.material.uniforms.uGridSize = new THREE.Uniform(
      this.gpgpuSize,
    );
    ...
    this.variable.material.uniforms.uDistance = new THREE.Uniform(
      this.settings.distance,
    );
    this.variable.material.uniforms.uRelaxation = new THREE.Uniform(
      this.settings.relaxation,
    );
  ...
  createMesh(w, h) {
    ...
    uniform float uColumns; // --- NEW: Added uColumns uniform ---
    varying vec2 vUv;
    uniform vec2 uMeshSize;
    uniform vec2 uImageResolution;

    // --- Classic GLSL Pseudo-Random Function ---
    ...
    void main() {
        ...
        // --- NEW: Replaced rectCount with the dynamic uColumns uniform ---
        float rectX = floor(vUv.x * uColumns) / uColumns;
        vec2 rectUv = vec2(rectX, vUv.y);

        // Sample the GPGPU physics using our vertical strip UVs
        vec4 displacement = texture2D(uGrid, rectUv);

        // THE RANDOM STAGGER
        // Generate a random number between 0.2 and 1.2 based on the column's position
        // We multiply rectX by an arbitrary number (12.9898) just to scramble the seed
        float randMultiplier = random(rectX * 12.9898) + 0.2;

        // Multiply the shift by our random stagger!
        vec2 shift = displacement.rg * vec2(1.0, 0.2) * 0.01 * randMultiplier;

        vec2 finalUvs = newUvs - shift;

        // RGB split for the visual effect
        float red = texture2D(uTexture, finalUvs + shift * 0.1).r;
        float green = texture2D(uTexture, finalUvs).g;
        float blue = texture2D(uTexture, finalUvs - shift * 0.1).b;
...
`;
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColumns: new THREE.Uniform(this.settings.gridSize), // --- NEW: Registered uColumns ---
      },
    });
  ...
  render() {
    ...
    this.variable.material.uniforms.uDeltaMouse.value.multiplyScalar(
      this.settings.relaxation,
    );

    this.variable.material.uniforms.uDistance.value = this.settings.distance;
    this.variable.material.uniforms.uRelaxation.value =
      this.settings.relaxation;

    // --- NEW: Sync visual columns to the physics grid size every frame ---
    this.material.uniforms.uColumns.value = this.settings.gridSize;

    this.gpgpuRenderer.compute();

    const renderTarget = this.gpgpuRenderer.getCurrentRenderTarget(
      this.variable,
    );
    this.material.uniforms.uGrid.value =
      renderTarget.texture || renderTarget.textures[0];
